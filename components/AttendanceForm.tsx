...
import { attendanceData, IEVFormProps } from "@/data/attendance_questions";
...
const FormTemplate = (props: IEVFormProps) => {
  ...
  const handleSubmit: (prevState: any, formData: FormData) => Promise<DbResponse> = async (prevState: any, formData: FormData) => {
    const saveData: DbResponse = await fetch("/api/file_upload_attendance", {
      method: "POST",
      body: formData
    })
