...
import { attendanceSchema } from "@/zod_schema"
...
const saveToDb = async (form: CastedFormInterface, imageFileName: string) => {
  ...
  try {
    ...
    const addEntry = await db.lga_assistant_20250403.create({ 
      data: {
        ...dbData
      }
    });
    ...
    console.log("Your attendance has been taken!")
  } catch (error) {
    console.error(error)
...
export const POST = async (request: NextRequest) => {
  try {
    ...
    const castedForm: CastedFormInterface = {
      ...
      network: filledForm.get("network") as string || "",
    ...
    const validatedForm = attendanceSchema.safeParse(castedForm);
